// Exact integration of the user's provided module with no logic changes
// Only change: import path for 'three' helper points to our local three wrapper
...
export default function (params: any) {
  ...
  const threeConfig: Record<string, any> = {}
  ...
  const api = three({
    ...
    antialias: false,
    initCamera (threeCtx) {
      threeCtx.camera = new OrthographicCamera()
    },
    initScene ({ scene }) {
    ...
    beforeRender ({ clock, width, height, wWidth }) {
      ...
      if (!hover) {
        ...
        uColor.value.r = 0.5 + 0.5 * Math.cos(tt * 0.0015)
        uColor.value.g = 0
